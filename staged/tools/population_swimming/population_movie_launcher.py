"""
population_movie_launcher.py
============================
GUI entry point for population_movie.py -- no command line, no typed filenames.
...
LOG_PATH = Path(__file__).resolve().parent / "population_movie_launcher.log"
...
def main():
    try:
        ...
        import population_movie as km
    except Exception:
        _log_and_show("Population Results Movie -- startup failed",
    ...
    try:
        ...
        csv_path = filedialog.askdirectory(
            parent=root,
            title="Choose the population-tracking RESULTS folder")
        if not csv_path:
            root.destroy()
            return
        ...
        image_path = filedialog.askopenfilename(
            parent=root,
            title="Choose the recording it was tracked on "
        ...
        root.deiconify()
        root.title("Population results movie")
        ...
        flagged = q.get("rows_crossing_ambiguous", 0)
        summary = (f"{rec.n_frames} frames  |  {rec.n_seg} segments  |  "
                   f"{rec.fps:g} fps declared  |  "
                   f"{len(rec.images) if rec.images else 0} images\n"
                   f"{q.get('n_bouts', 0)} modality bouts, median confidence "
                   f"{q.get('median_bout_confidence', 0):.2f}")
            ...
            def work():
                try:
                    ...
                    if preview_only:
                        ...
                        p, prov = km.render(
                            rec, out_dir / f"{rec.base}_population_movie.mp4",
                    ...
                    _log_and_show("Population Results Movie -- render failed",
                                  traceback.format_exc())
                finally:
                    for child in buttons.winfo_children():
                        child.state(["!disabled"])
        ...
        _log_and_show("Population Results Movie -- error",
