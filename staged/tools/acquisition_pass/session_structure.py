...
# fc2_save_2021-04-19-132028-0000.tif  -> FlyCapture2's own save pattern.
FLYCAP_RE = re.compile(
    r"^fc2_save_(\d{4})-(\d{2})-(\d{2})-(\d{6})-(\d+)", re.I)
...
def session_key(name):
    """(session id, frame index) for one filename, or None."""
    match = FLYCAP_RE.match(name)
    if match:
        stamp = f"{match.group(1)}-{match.group(2)}-{match.group(3)}-" \
                f"{match.group(4)}"
        return stamp, int(match.group(5))
...
def analyse(folder, names):
    ...
    for name in names:
        ...
        if key:
            sessions[key[0]].append(key[1])

    if not sessions:
        return []

    stamped = {s: seconds_of(s) for s in sessions}
...
def main():
    ...
    fields = ["folder", "session", "frames", "start_s", "gap_to_next_s",
              ...
              "rate_measured", "duration_s", "sessions_in_folder",
    ...
    print(f"{'SESSIONS (the actual recordings)':32} {len(rows):,}")
